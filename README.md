...
| 配置项 | 说明 |
|---|---|
| **总开关** `looklook.enabled` | ON（默认）= 全部能力可用；OFF = 插件休眠，不拦截任何入口，`looklook_see` 答"已关闭"，DSH 恢复原样（不卸载） |
| **视觉模型** | 识别图片/视频画面（视频共用）；主+备用故障切换 |
| **音频模型** | 对白+声音理解（自动探测能力）；主+备用 |
| **本地 ASR 一键安装** | 安装 faster-whisper；可选 tiny/base/small/medium/large-v3，**同时只保留一个模型**（换装即清除旧模型），无需 API |
