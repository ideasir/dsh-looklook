...
export function isZipContainer(data: Uint8Array): boolean {
 ...
 */
export async function loadPackage(data: Uint8Array): Promise<OfficePackage> {
  ...
  await new Promise<void>((resolve, reject) => {
    unzip(data, {
      ...
      for (const [path, bytes] of Object.entries(unzipped as Unzipped)) {
...
export function readPartText(pkg: OfficePackage, path: string): string | undefined {
 ...
 */
export function resolveRelationship(pkg: OfficePackage, partPath: string, rId: string): string | undefined {
  ...
  while ((element = elementRe.exec(xml)) !== null) {
    ...
    if (idMatch === null || idMatch[1] !== rId) continue
...
export function mediaTypeForPart(partPath: string): string {
 ...
 * dropped.
 */
export function bytesMatchMediaType(data: Uint8Array, mediaType: string): boolean {
