...
import type { ParsedDocument, DocSection, ExtractedImage } from './types.ts'
...
/** One resolved sheet: its name, part path, and grid. */
interface SheetInfo {
  name: string
  part: string
}
...
function isDateCell(cell: XmlNode, dateStyles: ReadonlySet<number>): boolean {
  const styleAttr = attr(cell, 's')
  if (styleAttr === undefined || styleAttr.length === 0) return false
 ...
 */
function dateStyleIndices(pkg: OfficePackage): Set<number> {
  const indices = new Set<number>()
  const xml = readPartText(pkg, 'xl/styles.xml')
  if (xml === undefined) return indices
...
class SheetImages {
   ...
   */
  resolveAll(pkg: OfficePackage, sheetPart: string, rId: string, location: string): number[] {
  ...
  private add(pkg: OfficePackage, part: string, location: string): number | undefined {
...
function collectBlipRefs(node: unknown, out: string[]): void {
  walk(node, (tag, element) => {
    if (tag === 'a:blip') {
      const rId = attr(element, 'r:embed')
      if (rId !== undefined && rId.length > 0) out.push(rId)
    }
  })
}

/** Parse one cell node into its display value. */
function cellValue(cell: XmlNode, shared: SharedStrings, dateStyles: ReadonlySet<number>): string {
 ...
 */
function rowCells(row: XmlNode, shared: SharedStrings, dateStyles: ReadonlySet<number>): string[] {
  const values: string[] = []
  for (const cell of children(row, 'c')) {
    const ref = attr(cell as XmlNode, 'r') ?? ''
    const col = columnIndex(ref)
    if (col >= 0) {
      // Pad any skipped columns so sparse rows keep their alignment.
      while (values.length < col) values.push('')
      values[col] = cellValue(cell as XmlNode, shared, dateStyles)
    } else {
      // No usable reference: fall back to sequential placement.
      values.push(cellValue(cell as XmlNode, shared, dateStyles))
    }
  }
  return values
}
...
function mergedCellRefs(ws: unknown): string[] {
  const refs: string[] = []
  walk(ws, (tag, element) => {
    if (tag === 'mergeCell') {
      const ref = attr(element, 'ref')
      if (ref !== undefined && ref.length > 0) refs.push(ref)
    }
  })
  return refs
}
...
function listSheets(pkg: OfficePackage): SheetInfo[] {
 ...
 */
export function parseXlsx(pkg: OfficePackage): ParsedDocument {
  ...
  for (const sheet of sheets) {
    ...
    for (const row of children(sheetData, 'row')) {
      const values = rowCells(row as XmlNode, shared, dateStyles)
      // Trim trailing empty cells so the grid renders cleanly.
      while (values.length > 0 && values[values.length - 1] === '') values.pop()
      if (values.length > 0) rows.push(values)
    }
    if (mergeRefs.length > 0) expandMergedCells(rows, mergeRefs)

    if (rows.length === 0 && refs.length === 0) continue
...
function findSheetDrawingRId(ws: unknown): string | undefined {
  const drawing = child(ws, 'drawing')
  const node = asArray(drawing)[0] as XmlNode | undefined
