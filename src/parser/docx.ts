...
class ImageRegistry {
  ...
  resolve(pkg: OfficePackage, relsPart: string, rId: string, location: string): number | undefined {
...
function findImageRefs(node: unknown): string[] {
  const refs: string[] = []
...
function paragraphText(paragraph: unknown): string {
  ...
  walk(paragraph, (tag, element) => {
    ...
    if (tag === 'w:delText' || tag === 'w:delInstrText') return
    if (tag === 'w:t') {
...
function tableRows(pkg: OfficePackage, relsPart: string, table: unknown, images: ImageRegistry, location: string): { rows: string[][]; refs: number[] } {
  const rows: string[][] = []
  const refs: number[] = []
  for (const tr of children(table, 'w:tr')) {
    ...
    for (const tc of children(tr, 'w:tc')) {
      ...
      for (const rId of findImageRefs(tc)) {
        ...
        if (index !== undefined && !refs.includes(index)) refs.push(index)
      }
 ...
 */
export function parseDocx(pkg: OfficePackage): ParsedDocument {
  ...
  let tableIndex = 0
  for (const [tag, value] of Object.entries(bodyNode as Record<string, unknown>)) {
    ...
    if (tag === 'w:p') {
      for (const p of asArray(value)) {
        ...
        const refs = findImageRefs(p).map(rId => images.resolve(pkg, 'word/document.xml', rId, `第 ${paragraphIndex} 段`)).filter((i): i is number => i !== undefined)
  ...
  for (const part of headerFooterParts) {
    ...
    const refs = findImageRefs(root).map(rId => images.resolve(pkg, part, rId, label)).filter((i): i is number => i !== undefined)
...
function paragraphStyle(paragraph: unknown): string {
  let style = ''
  walk(paragraph, (tag, element) => {
    if (tag === 'w:pStyle') {
...
function findHeaderFooterParts(pkg: OfficePackage, documentXml: string): string[] {
  ...
  walk(bodyNode, (tag, element) => {
    if (tag === 'w:headerReference' || tag === 'w:footerReference') {
      ...
      if (target !== undefined) parts.add(target)
    }
  })
  ...
  if (parts.size === 0) {
    for (const path of pkg.entries.keys()) {
      if (/^word\/(header|footer)\d+\.xml$/u.test(path)) parts.add(path)
    }
  }
  return [...parts].sort()
}
