...
export function asrModelOption(id) {
 ...
 */
export async function ensureVenv(basePython, venvDir) {
...
export async function installedAsrModel() {
 ...
 */
async function purgeOtherModels(keepModel) {
...
function transcribeScript(modelId) {
    return `import sys
try:
...
export async function performInstall(modelId = DEFAULT_ASR_MODEL) {
    ...
    if (installState.phase === 'checking' || installState.phase === 'installing-deps'
        || installState.phase === 'downloading-model' || installState.phase === 'writing')
        return;
    ...
    try {
        ...
        if (!py.ok)
            throw new Error(`Python 不可用（${pythonCmd}）：${py.stderr || '运行失败'}`);
        ...
        if (venvPy === undefined)
            throw new Error('创建隔离 Python 环境失败');
...
export function currentInstallPhase() {
    return installState.phase;
}
/** Read the last install error, if any. */
export function currentInstallError() {
