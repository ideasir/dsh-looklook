...
let LooklookRemoteService = (() => {
    ...
    let _uninstallLooklook_decorators;
    return class LooklookRemoteService extends _classSuper {
        static {
            ...
            _uninstallLooklook_decorators = [Remote];
        ...
        async uninstallLooklook() {
            try {
                const { existsSync, unlinkSync, readFileSync, writeFileSync } = await import('node:fs');
                const results = [];
                // 清理 settings.yaml 中的 looklook 配置
                const settingsPath = '/root/.dsh/settings.yaml';
                if (existsSync(settingsPath)) {
                    try {
                        let raw = readFileSync(settingsPath, 'utf-8');
                        raw = raw.replace(/^looklook:\s*\n([\s\S]*?)(?=\n[a-zA-Z]|\n$)/m, '');
                        writeFileSync(settingsPath, raw, 'utf-8');
                        results.push('settings.yaml(looklook段)');
                    } catch {}
                }
                if (results.length > 0) {
                    return { ok: true, text: `已清理: ${results.join(', ')}。请删除 dsh-looklook 插件目录并重启 DSH。` };
                }
                return { ok: true, text: '未发现 LookLook 相关配置文件' };
            } catch (e) {
