...
import { enabledProviders, enabledAudioProviders } from "./settings.js";
import { describeImages } from "./vision-client.js";
import { readDocument } from "./parser/index.js";
import { parsePdf, MAX_PDF_PAGES } from "./parser/pdf.js";
import { parsePsd, loadCompositePreview, encodePng } from "./parser/index.js";
import { bytesMatchMediaType } from "./parser/package.js";
...
// ── Shared helpers ──
async function resolveApiKeyOf(ctx) {
...
async function readFileBytes(ctx, path, cwd, maxBytes, signal) {
    const fs = ctx.get('fs');
    if (fs === undefined)
        throw new Error('fs 服务不可用');
    const target = await fs.resolve(path, { ...cwd === undefined ? {} : { cwd }, signal });
    const info = await fs.stat(target, signal);
    if (info === undefined)
        throw new Error(`无法读取 "${target.displayPath}"：文件不存在`);
    if (info.type !== 'file')
        throw new Error(`无法读取 "${target.displayPath}"：不是普通文件`);
...
async function describeImagesOf(ctx, visionScope, images, signal) {
    ...
    const warnings = [];
    if (unsupported > 0)
        warnings.push(`${unsupported} 张图片为矢量图或未支持格式，未做视觉识别`);
    if (mismatched > 0)
        warnings.push(`${mismatched} 张图片内容与扩展名不符，未做视觉识别`);
    if (skippedByCap > 0)
        warnings.push(`${skippedByCap} 张图片超过单次识别上限，未做视觉识别`);
...
async function identifyBackgroundMusic(ctx, audioScope, audio, signal) {
...
function renderOfficeDigest(doc, descriptions, maxChars) {
    const lines = [];
    const formatLabel = { docx: 'Word 文档', xlsx: 'Excel 表格', pptx: 'PPT 演示文稿' };
    ...
    for (const section of doc.sections) {
        ...
        if (prefix.length > 0 || title.length > 0)
            block.push(`${prefix}${title}`.trim());
        if (section.text.length > 0)
            block.push(section.text);
...
// ── PDF ──
function renderPdfDigest(parsed, descriptions, maxChars) {
    const lines = [];
    lines.push('# PDF 文档内容');
...
// ── PSD ──
function renderPsdDigest(psd, overallDescription, maxChars) {
    const lines = [];
    lines.push('# PSD 文档分析');
    lines.push(`- 画布：${psd.width} × ${psd.height}px${psd.resolution !== undefined ? `（${psd.resolution}dpi）` : ''}`);
    if (psd.colorMode !== undefined)
        lines.push(`- 色彩模式：${psd.colorMode}`);
    ...
    lines.push('');
    lines.push('## 图层结构');
    if (psd.tree.length === 0)
        lines.push('（空文档）');
    for (const node of psd.tree)
        renderPsdNode(node, 0, lines);
    return lines.join('\n').slice(0, maxChars);
}
function renderPsdNode(node, depth, out) {
    ...
    if (node.type === 'group') {
        ...
        for (const childNode of node.children ?? [])
            renderPsdNode(childNode, depth + 1, out);
        return;
    }
...
const DOC_EXTENSIONS = ['.docx', '.xlsx', '.pptx', '.pdf', '.psd'];
 ...
 */
export async function readDocumentFile(ctx, visionScope, audioScope, source, question, cwd, signal) {
        ...
        if (lower.endsWith('.pdf')) {
            ...
            if (providers.length > 0) {
                ...
                for (const page of parsed.pages) {
                    if (signal.aborted)
                        break;
                    if (!page.isScan || page.images.length === 0)
                        continue;
                    const inputs = page.images
                        .slice(0, 4)
                        .map(image => ({ mediaType: 'image/png', data: image.data }));
                    if (inputs.length === 0)
                        continue;
                    const result = await describeImages(providers, resolveApiKey, inputs, 2000, signal);
                    if (result.ok)
                        descriptions.set(page.pageNumber, result.text);
                }
            }
        ...
        if (lower.endsWith('.psd')) {
            ...
            if (providers.length > 0 && psd.width * psd.height <= 40_000_000) {
                ...
                if (preview !== undefined) {
                    ...
                    if (result.ok)
                        overallDescription = result.text;
                }
            }
