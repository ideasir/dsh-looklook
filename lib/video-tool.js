...
import { audioEnabled, enabledAudioProviders } from "./settings.js";
 ...
 */
async function understandAudio(videoPath, providers, resolveApiKey, question, signal) {
    ...
    try {
        ...
        if (!response.ok)
            return { ok: false, error: `音频理解失败（HTTP ${response.status}）` };
        const body = await response.json();
        const text = typeof body.choices?.[0]?.message?.content === 'string'
            ? body.choices[0].message.content.trim()
            : '';
        if (text === '')
            return { ok: false, error: '音频模型返回了空内容' };
        return { ok: true, text };
    }
    catch (error) {
...
function composeReport(out, question, audio) {
    const parts = [];
...
export function registerWatchTool(ctx, audioScope) {
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (source === '')
                return { text: '看视频失败：缺少 source 参数（视频文件路径或链接）' };
