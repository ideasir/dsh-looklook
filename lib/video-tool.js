 ...
 */
export function configuredVideoProxy(env = process.env) {
 ...
 */
async function resolveWorkerPython() {
    // Venv exists?
    try {
        ...
        if (!pyEnv.ok || pyEnv.command === undefined)
            return { ok: false, error: pyEnv.error };
        ...
        if (ch === '{') {
            ...
            if (depth === 0 && start >= 0) {
                try {
                    ...
                    if (candidate !== null && typeof candidate === 'object' && typeof candidate.ok === 'boolean')
                        last = candidate;
                }
                catch {
 ...
 */
export async function watchVideo(ctx, audioScope, visionScope, source, question, signal) {
    if (source === '')
