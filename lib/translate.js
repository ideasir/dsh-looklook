...
import { describeImages, statusMessage } from "./vision-client.js";
...
export function describeResultText(result) {
    if (result.ok) {
...
function rewriteContent(blocks, textFor) {
    const out = [];
...
export function replaceImagesWithPlaceholder(messages) {
    if (!messagesHaveImage(messages))
        return messages;
    return messages.map(message => (contentHasImage(message.content)
...
function cachedDescriptions(session) {
    const cache = new Map();
    for (const event of session.events) {
        if (event.type !== 'vision/describe')
            continue;
        const record = event.data;
        if (record.ok && record.text !== undefined)
            cache.set(record.attachmentId, record);
    }
    return cache;
}
/**
 ...
 */
function recordDescribe(ctx, sessionId, data) {
    ...
    const event = session.append('vision/describe', data, { ignorable: true });
    if (event.ignorable !== true) {
        ctx.logger.warn('dsh-looklook: this DSH build does not persist plugin events (ignorable append unsupported); recognition cache lives in memory only — apply the compatibility patch or upgrade DSH');
    }
}
/** Eye-on, text-only path: describe every image and replace it with the result text. */
export async function translateImages(ctx, messages, sessionId, scope, signal) {
    const session = sessionId === undefined ? undefined : ctx.sessions.get(sessionId);
    const cache = session === undefined ? new Map() : cachedDescriptions(session);
    ...
    const textFor = async (image) => {
        const cached = cache.get(String(image.attachment.attachmentId));
        if (cached !== undefined && cached.text !== undefined)
            return cached.text;
...
async function rewriteContentAsync(blocks, textFor) {
    const out = [];
    for (const block of blocks) {
        if (block.type === 'image') {
