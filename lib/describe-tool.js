 ...
 */
function resolveRef(raw, registry) {
...
export function registerDescribeTool(ctx, scope, refRegistry, features) {
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if ('error' in resolved)
                return { text: '识图失败：' + resolved.error };
            try {
                ...
                const result = await describeImages(providers, resolveApiKey, [{ mediaType: stored.ref.mediaType, data: stored.data }], maxChars, exec.signal, question);
                if (!result.ok)
                    return { text: '识图失败：' + result.message };
