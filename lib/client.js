	...
	factory: (require) => {
		...
		*/
		...
		function LooklookPluginCard(props) {
			const { api, pluginSettings, t, features, useFeatures, listModels, testVision, testAudio, asrStatus, asrInstall, envCheck, envRepair, usePluginEnabled, uninstallLooklook } = props;
			const [open, setOpen] = (0, react.useState)(false);
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							style: css.headText,
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								style: { ...css.name, display: "flex", alignItems: "baseline", gap: "6px" },
								children: [title, /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "dsh-ll-badge",
									style: { color: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.2)", cursor: "default", fontSize: "9px", lineHeight: "12px", padding: "0px 4px" },
									children: "0819-rc.7"
								})]
							}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: css.desc,
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("a", {
								...
								onClick: (e) => {
									e.stopPropagation();
									if (!confirm("确定要卸载 dsh-looklook 插件吗？\n\n将删除所有 LookLook 配置。\n删除后需要重启 DSH 生效。")) return;
									if (uninstallLooklook) {
										uninstallLooklook().then((r) => {
											alert(r.ok ? "✅ " + (r.text || "卸载完成") : "卸载失败: " + (r.error || "未知错误"));
										}).catch((err) => {
								...
								className: "dsh-ll-uninstall",
								style: { border: "1px solid rgba(248,81,73,0.4)", background: "transparent", color: "rgba(248,81,73,0.9)" },
								children: "卸载"
							})]
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							style: {
		...
		*/
		function apply(ctx) {
			...
			// 注入 hover CSS
			if (typeof document !== "undefined" && !document.getElementById("dsh-looklook-css")) {
				const el = document.createElement("style");
				el.id = "dsh-looklook-css";
				el.textContent = [
					".dsh-ll-badge { font-size:10px; font-weight:600; padding:0px 5px; border-radius:4px; line-height:14px; letter-spacing:0.5px; text-decoration:none; cursor:pointer; transition:all .15s; }",
					".dsh-ll-badge:hover { background:rgba(255,255,255,0.15); }",
					".dsh-ll-uninstall { font-size:10px; padding:1px 5px; border-radius:4px; cursor:pointer; line-height:14px; border:none; transition:all .15s; }",
					".dsh-ll-uninstall:hover { background:rgba(248,81,73,0.2); }"
				].join("\n");
			...
			// 注入消息操作栏复制按钮（每条助手消息的复制/分支图标旁边）
			ctx.slots.inject("conversation.chat.assistant-actions", () => ctx.slots.register({
				name: "conversation.chat.assistant-actions",
				id: "looklook-copy-session",
				priority: 50,
				inject: ({ messageId }) => ({ messageId })
			}, function CopySessionButton() {
				const sessionsRef = ctx.get("sessions");
				const [copied, setCopied] = (0, react.useState)(false);
				const snapshot = (0, react.useSyncExternalStore)(
					(listener) => sessionsRef?.list?.subscribe?.(listener) ?? (() => {}),
					() => sessionsRef?.list?.getSnapshot?.() ?? { byId: {}, current: undefined }
				);
				const sessionId = snapshot?.current;
				const title = sessionId ? (snapshot.byId?.[sessionId]?.title ?? "") : "";
				const handleClick = () => {
					if (!sessionId) return;
					const ref = "dsh-session://" + sessionId;
				...
				return (0, react_jsx_runtime.jsx)("button", {
					type: "button",
					className: "p-xYUq_action",
					onClick: handleClick,
					title: copied ? "已复制" : "复制会话引用",
					"aria-label": copied ? "已复制" : "复制会话引用",
					style: { color: copied ? "#3fb950" : undefined },
					children: copied
						? (0, react_jsx_runtime.jsx)("svg", { width: 16, height: 16, viewBox: "0 0 16 16", fill: "none", children: (0, react_jsx_runtime.jsx)("path", { d: "M13.5 4.5L6.5 11.5L2.5 7.5", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round" }) })
						: (0, react_jsx_runtime.jsxs)("svg", { width: 16, height: 16, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", children: [(0, react_jsx_runtime.jsx)("path", { d: "M20 4v7a4 4 0 0 1-4 4H4" }), (0, react_jsx_runtime.jsx)("path", { d: "m9 10-5 5 5 5" })] })
				});
			}));
			...
			*/
			...
			const ensureSubmitPatched = (sessionId) => {
				...
				if (actx === void 0) {
					console.warn("[looklook] ensureSubmitPatched: actx undefined for", sessionId, "sessions.scope exists:", !!sessions.scope);
					return;
				}
				const conversation = actx.get?.("conversation");
				if (conversation === void 0) {
					console.warn("[looklook] ensureSubmitPatched: conversation service undefined");
					return;
				}
				if (conversation.input === void 0) {
					console.warn("[looklook] ensureSubmitPatched: conversation.input undefined");
					return;
				}
				const shell = conversation.input.for?.(actx);
				if (shell === void 0) {
					console.warn("[looklook] ensureSubmitPatched: shell undefined from conversation.input.for");
					return;
				}
				if (shell.submit === void 0 || shell.setDraft === void 0 || shell.state === void 0) {
					console.warn("[looklook] ensureSubmitPatched: shell missing submit/setDraft/state", {
						submit: !!shell.submit,
				...
				patchedSessions.add(sessionId);
				console.log("[looklook] submit patched for session", sessionId);
			};
			ctx.effect(() => {
				const sync = () => {
			...
			ctx.effect(() => {
				const mounting = ctx.remote.$mount({
					...
					descriptors: [
						...
						{
							id: "looklook.uninstallLooklook",
							service: "looklookRemote",
							namespace: "looklook",
							method: "uninstallLooklook",
							invocation: { kind: "direct" },
							parameters: [],
							result: {
								mode: "strict",
								typeSymbol: "LooklookUninstallResult",
								schema: { parse: parseAsIs }
							}
						}
					]
			...
			*  the previous one. */
			...
			const uninstallLooklook = async () => {
				const remote = ctx.get("remote.looklook");
				if (remote?.uninstallLooklook === void 0) return { ok: false, error: "卸载服务未就绪" };
				const envelope = await remote.uninstallLooklook();
				if (!envelope.ok) return { ok: false, error: typeof envelope.error === "string" ? envelope.error : envelope.error?.message ?? "卸载失败" };
				return envelope.value ?? { ok: true, text: "卸载完成" };
			};
			...
			ctx.slots.inject("conversation.input.dock", () => ctx.slots.register({
				...
				id: PENDING_ID,
				inject: (sessionId) => {
					console.log("[looklook] dock inject called, sessionId:", sessionId, "pending:", pending.get(sessionId)?.length ?? 0);
