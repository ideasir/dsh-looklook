		...
		function apply(ctx) {
			...
			*/
			const wrapSubmit = () => {
				...
				const shell = ((sessions.scope ? sessions.scope?.(sessionId) : void 0)?.get?.("conversation"))?.input?.shell?.(sessionId);
				if (shell?.submit === void 0 || shell?.setDraft === void 0) return false;
				const wrapped = shell;
