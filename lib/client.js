window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		async function uploadFiles(sessionId, files, buildNote) {
			const lines = [];
		...
		*/
		function apply(ctx) {
			...
			const stageIntoDraft = (notes) => {
				...
				if (inputActions?.setDraft === void 0) return;
			...
			ctx.effect(() => {
				...
				const onDropCapture = (event) => {
					...
					(async () => {
						const notes = await uploadFiles(sessionId, files, (name, path) => t("upload.message", {
							name,
							path
						}));
