		...
		*/
		function apply(ctx) {
			...
			ctx.slots.inject("conversation.chat.assistant-actions", () => ctx.slots.register({
				...
				const handleClick = () => {
					if (!messageId) return;
					const sessionsList = sessionsRef?.list?.();
					const session = sessionsList?.find((s) => s.sessionId === messageId || s.messages?.some?.((m) => m.id === messageId));
				...
				return (0, react_jsx_runtime.jsx)("button", {
					...
					style: {
						...
						color: copied ? "#3fb950" : "var(--dsw-alias-label-secondary, #8b949e)",
						cursor: "pointer",
						transition: "all .15s",
						whiteSpace: "nowrap"
					},
					children: copied ? "✅ 已复制" : "📋 复制会话引用"
				});
			}));
