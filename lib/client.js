window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		var upload_shared_exports = /* @__PURE__ */ __exportAll({
			ACCEPT_EXTENSIONS: () => ACCEPT_EXTENSIONS,
			fileToBase64: () => fileToBase64,
			isUploadableName: () => isUploadableName,
		...
		*/
		function apply(ctx) {
			...
			const wrapSubmit = () => {
				...
				const actions = current?.props?.inputActions;
				if (sessionId === void 0 || actions?.submit === void 0 || actions?.setDraft === void 0) return;
				...
				wrapped.submit = () => {
					...
					if (staged.length > 0) {
						const notes = staged.map((f) => t("upload.message", {
							name: f.name,
							path: f.path
						})).join("\n");
						const draft = (current?.hooks?.input)?.getSnapshot()?.draft ?? "";
				...
				const onDropCapture = (event) => {
					...
					(async () => {
						const results = await Promise.all(files.map(async (file) => {
							try {
								const { path } = await Promise.resolve().then(() => upload_shared_exports).then((m) => m.uploadFile(sessionId, file));
								return {
									name: file.name,
									path,
									size: file.size
						...
						for (const result of results) if (result !== null) pending.add(sessionId, result);
					})();
				};
