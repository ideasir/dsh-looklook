...
import { configuredVideoProxy } from "./video-tool.js";
import { detectPython } from "./python-env.js";
import { localAsrReady, ensureVenv, VENV_DIR } from "./asr-install.js";
...
async function checkYtDlp(pythonCmd) {
    const probe = await runProbe(pythonCmd, ['-c', 'import yt_dlp.version; print(yt_dlp.version.__version__)'], 15_000);
    if (probe.ok) {
        return {
            id: 'yt-dlp',
            label: 'yt-dlp（视频链接下载）',
